...
{
  lidR:::assert_package_is_installed("hexbin")
  ...
  xmin <- lidR::round_any(ext@xmin, resx)
  xmax <- lidR::round_any(ext@xmax, resx)
  ymin <- lidR::round_any(ext@ymin, resy)
  ymax <- lidR::round_any(ext@ymax, resy)
  if (xmax < ext@xmax)
    xmax <- xmax + resx
  if (xmin > ext@xmin)
    xmin <- xmin - resx
